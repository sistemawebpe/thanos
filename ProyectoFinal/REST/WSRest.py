...
def UpdateEstaciones():
    idEstacion=str(request.form.get("idEmpresa"))
    NomEstacion=request.form.get("NomEstacion")
    Telefono=str(request.form.get("Telefono"))
    Direccion=request.form.get("Direccion")
    codDistrito = request.form.get("codDistrito")
    ...
    sql="UPDATE upc_estaciones SET NomEstacion='" + NomEstacion + "', Direccion='"+Direccion+"',Telefono='"+Telefono+"',CodDistrito='"+ codDistrito +"',gas84plus='"+ gas84plus +"',gas90plus='" + gas90plus +"',gas95plus='"+gas95plus+"',gas97plus='"+gas97plus+"',gas98plus='"+gas98plus+"',dbs_s50_uv='"+dbs_s50_uv+"' WHERE idEstacion='"+idEstacion + "'"
...
@app.route('/EliminarEstaciones',methods=["POST"])
def EliminarEstaciones():
    idEstacion=str(request.form.get("idEmpresa"))
    resultado="Borrar"
    sql="SELECT  IF((gas84plus+ gas90plus + gas95plus + `gas97plus` + `gas98plus` +`dbs_s50_uv`)=0,'Borra','NoBorra') AS Resultado FROM upc_estaciones  WHERE idEstacion='" + idEstacion +"'"
    try:
        cur.execute(sql)
        datos=cur.fetchall()    
        resultado=datos[0][0]
        print "Resulado para borrar:" + resultado
    except mysql.connector.Error as err:
        print "Error al momento de consultar" + str(err)
        
    if resultado=="NoBorra":
        return jsonify({"Mensaje":"NoOK"})
    else:    
        sql="UPDATE upc_estaciones SET Estado='D' WHERE idEstacion='" + idEstacion + "'"
        try:
            cur.execute(sql)
            conn.commit()
            print "Se ingreso el distrito"
